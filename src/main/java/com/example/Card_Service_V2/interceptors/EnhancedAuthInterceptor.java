    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (authHeader == null || !authHeader.startsWith("Bearer ")) {
            ...
            String json = new com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(
                com.example.Card_Service_V2.utils.ApiResponse.error("Authorization header required", "Unauthorized")
            );
        ...
        if (!validationResponse.isValid()) {
            ...
            response.setContentType("application/json");
            String json = new com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(
                com.example.Card_Service_V2.utils.ApiResponse.error(validationResponse.getMessage(), "Unauthorized")
            );
            response.getWriter().write(json);
            return false;
        }
