...
public static class AccountInfoWithCurrency extends AccountInfo {
    ...
    private CardModel createCardModel(CreateCardRequestDTO request, CardSensitiveData sensitiveData) {
        ...
        card.setType(request.getType() != null ? request.getType().trim().toUpperCase() : null);
        card.setCurrency(request.getCurrency());
        card.setNetwork(request.getNetwork() != null ? request.getNetwork().trim().toUpperCase() : null);
   ...
   public boolean processInternalCardVerification(InternalCardVerificationRequestDTO request) {
    ...
    private boolean isDuplicateCard(CreateCardRequestDTO request) {
        ...
        String network = request.getNetwork() != null ? request.getNetwork().trim().toUpperCase() : null;
	...
	public Page<CardListDTO> processCardListingPaginated(String status, String type, Integer userId, Integer accountId,
			String network, boolean isAdmin, Integer tokenUserId, int page, int size) {
		int zeroBasedPage = Math.max(0, page - 1);
		Pageable pageable = PageRequest.of(zeroBasedPage, size);
		Page<CardModel> cardPage;
		if (isAdmin) {
			cardPage = repo.findWithFiltersPaged(status, type, userId, accountId, network, pageable);
		} else {
			cardPage = repo.findWithFiltersPaged(status, type, tokenUserId, accountId, network, pageable);
		}
		List<CardListDTO> dtoList = new ArrayList<>();
		for (CardModel card : cardPage.getContent()) {
			boolean showFullNumber = !isAdmin && tokenUserId != null && tokenUserId.equals(card.getUserid());
			dtoList.add(createCardListDTO(card, showFullNumber, tokenUserId));
		}
